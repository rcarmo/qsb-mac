...
/// This notification is sent when the database will be closing.  When
/// a new GTMSQLiteStatement is created, it will add an observer for
/// the owning SQLite database for this message.  This handshaking is
/// necessary because of the need to finalize the SQLite statement
/// before closing the database.
extern NSString *const kGTMSQLiteDatabaseWillClose;
...
@protected
  ...
  __weak GTMSQLiteDatabase *gtmdb_;
  BOOL hasCFAdditions_;
}
...
//
//  NOTE: Even though this object is autoreleased you really should call
...
//  you will almost always want to "finalize" the statement when you are
...
//    db: The GTMSQLiteDatabase (retained so that we have proper release
//        semantics around statement and database autorelease).
//    err:  Result code from SQLite. If nil is returned by this function
//          check the result code for the error. If NULL no result code is
//          reported.
//
...
//  Get the GTMSQLiteDatabase instance this statement is bound to.
//
//  Returns:
//    GTMSQLiteDatabase instance
//
- (GTMSQLiteDatabase *)database;
