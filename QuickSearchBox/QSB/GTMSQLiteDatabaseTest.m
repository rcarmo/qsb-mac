...
- (void)testInitAPI {
  ...
  STAssertNil(statement, @"Create statement succeeded with nil SQL string");
  STAssertNotEquals(err, SQLITE_OK, @"Err was not SQLITE_MISUSE on nil SQL "
                    @"string");
}
...
- (void)testParameterCountAPI {
  ...
  STAssertEquals(db, [statement database],
                 @"-[GTMSQLiteStatement database] "
                 @"API did not return out database :-(");
...
- (void) testResultCollectionAPI {
  ...
  GTM_FOREACH_OBJECT(db, databases) {
    ...
    // Check that we got at least one row back
    STAssertEquals(err, SQLITE_ROW, @"did not retrieve a row from db :-(");
    NSArray *rowArray = [statement resultRowArray];
    NSDictionary *dictionaryResults = [statement resultRowDictionary];
    NSLog(@"%@", rowArray);
    NSLog(@"%@", dictionaryResults);
  }
}
