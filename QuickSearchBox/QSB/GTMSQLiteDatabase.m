...
NSString * const kGTMSQLiteDatabaseWillCloseNotification =
  @"kGTMSQLiteDatabaseWillCloseNotification";
...
- (id)initWithPath:(NSString *)path
  ...
  if ((self = [super init])) {
    ...
    if (useUTF8) {
      ...
      NSMutableData *mutable = [[data mutableCopy] autorelease];
...
- (void)cleanupDB {
  if (db_) {
    [[NSNotificationCenter defaultCenter]
      postNotificationName:kGTMSQLiteDatabaseWillCloseNotification
                    object:self];
...
@interface GTMSQLiteStatement(PrivateMethods)
- (void)databaseWillClose:(NSNotification *)note;
@end
    ...
    if (sql && gtmdb) {
      ...
      if (sql) {
        ...
        if (rc == SQLITE_OK) {
          ...
          // Because the database will not close with any outstanding statements, we
          // need to finalize our statement when the database is trying to close.
          [defaultCenter addObserver:self
                            selector:@selector(databaseWillClose:)
                                name:kGTMSQLiteDatabaseWillCloseNotification
                              object:gtmdb_];
        } else {
          [self release];
          obj = nil;
...
#if GTM_SUPPORT_GC
- (void)finalize {
  // Start by finalizing the statement
